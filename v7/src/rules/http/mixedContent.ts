...
const selectors = ['script[src]', 'link[href]', 'img[src]', 'iframe[src]', 'video[src]', 'audio[src]', 'source[src]', 'embed[src]', 'object[data]']
...
export const mixedContentRule: Rule = {
  ...
  async run(page) {
    ...
    return {
      ...
      type: 'error',
      priority: 80,
      details: { ...buildDetails(offenders, paths), reference: SPEC, fix: 'Serve all subresources over HTTPS or remove them.' },
    }
  },
}
