...
export const gscPropertyAvailableRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    if (!r.ok) return { label: 'GSC', message: `GSC error ${r.status}`, type: 'warn' }
    const j = await r.json() as { siteEntry?: Array<{ siteUrl?: string }> }
    const ok = (j.siteEntry || []).some(s => (s.siteUrl || '').includes(origin))
    return ok ? { label: 'GSC', message: 'Property available', type: 'ok' } : { label: 'GSC', message: 'Property not available', type: 'warn' }
  },
}
