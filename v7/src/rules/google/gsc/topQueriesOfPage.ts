...
const TESTED = 'Queried GSC Search Analytics (dimensions: query,page) for the top queries of the URL.'
...
export const gscTopQueriesOfPageRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    return { label: 'GSC', message: `Top queries: ${rows || 'none'}`, type: 'info', name: NAME, details: { url: page.url, property, propertyType, topQueries: j.rows, apiResponse: j, tested: TESTED, reference: SPEC } }
  },
}
