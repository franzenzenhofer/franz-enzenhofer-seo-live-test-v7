...
export const gscIsIndexedRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    if (!r.ok) return { label: 'GSC', message: `GSC query error ${r.status}`, type: 'warn', name: "googleRule", details: { url: page.url, property, propertyType, status: r.status } }
    const j = await r.json() as { rows?: Array<{ clicks?: number, impressions?: number }> }
    const imp = (j.rows || []).reduce((a, x)=> a + (x.impressions || 0), 0)
    return imp > 0
      ? { label: 'GSC', message: `Indexed (impressions ${imp})`, type: 'ok', name: "googleRule", details: { url: page.url, property, propertyType, impressions: imp, apiResponse: j } }
      : { label: 'GSC', message: 'No impressions (might not be indexed)', type: 'warn', name: "googleRule", details: { url: page.url, property, propertyType, impressions: 0, apiResponse: j } }
  },
}
