...
export const gscIsIndexedRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    return imp > 0 ? { label: 'GSC', message: `Indexed (impressions ${imp})`, type: 'ok', name: '$(basename ${f%.ts})' } : { label: 'GSC', message: 'No impressions (might not be indexed)', type: 'warn', name: '$(basename ${f%.ts})' }
  },
}
