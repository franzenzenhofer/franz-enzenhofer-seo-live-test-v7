...
const TESTED = 'Queried GSC Search Analytics impressions for the exact page URL to infer indexing.'
...
export const gscIsIndexedRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    return imp > 0
      ? { label: 'GSC', message: `Indexed (impressions ${imp})`, type: 'ok', name: NAME, details: { url: page.url, property, propertyType, impressions: imp, apiResponse: j, tested: TESTED, reference: SPEC } }
      : { label: 'GSC', message: 'No impressions (might not be indexed)', type: 'warn', name: NAME, details: { url: page.url, property, propertyType, impressions: 0, apiResponse: j, tested: TESTED, reference: SPEC } }
  },
}
