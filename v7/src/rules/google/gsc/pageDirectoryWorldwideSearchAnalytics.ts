...
const TESTED = 'Queried GSC Search Analytics for aggregated impressions of the URL directory.'
...
export const gscDirectoryWorldwideRule: Rule = {
  ...
  async run(page, ctx) {
    ...
    if (!r.ok) return { label: 'GSC', message: `GSC query error ${r.status}`, type: 'warn', name: NAME, details: { url: page.url, property, propertyType, directory: dir, status: r.status, tested: TESTED, reference: SPEC } }
    ...
    return {
      ...
      type: 'info',
      name: NAME,
      details: { url: page.url, property, propertyType, directory: dir, impressions: imp, apiResponse: j, tested: TESTED, reference: SPEC },
    }
  },
}
