...
import { noteProbeResponse } from './siteProbeQueue'
 ...
 */
export const followChain = async (startUrl: string, opts: FollowOptions = {}): Promise<FollowResult> => {
  const maxHops = opts.maxHops ?? REDIRECT_MAX_HOPS
  const timeoutMs = opts.timeoutMs ?? REDIRECT_TIMEOUT_MS
  const fetchFn = probeFetch(opts)
  ...
  if (observer) {
    const observed = await followViaObserver(startUrl, { maxHops, timeoutMs, fetchFn, wantBody: opts.wantBody, signal: opts.signal }, observer)
    if (observed) return observed
  }
  const scope = abortScope(timeoutMs, opts.signal)
  const chain = newChain(startUrl, maxHops)
  const seen = new Set<string>([startUrl])
  let url = startUrl
  try {
    for (;;) {
      const res = await hopFetch(fetchFn, url, scope.signal, timeoutMs, chain.hops)
      if (res.type === 'opaqueredirect') {
        discardBody(res)
        return await followHidden(chain, url, fetchFn, scope.signal, opts.wantBody)
      }
