...
export class Logger {
  ...
  private static async getTabId(): Promise<number | null> {
    ...
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      try { const tabs = await chrome.tabs.query({ active: true, currentWindow: true }); if (tabs[0]?.id) return tabs[0].id } catch { /* ignore */ }
    }
    return null
  }
  ...
  static logSync(category: LogCategory, action: string, data?: LogData): void {
    ...
    try {
      if (typeof chrome !== 'undefined' && chrome.runtime) {
        chrome.runtime.sendMessage({ channel: 'log', tabId: this.currentTabId, message }).catch(() => {
          console.log(`[Logger] ${message}`)
        })
      }
    } catch {
  ...
  static async logDirect(tabId: number, category: LogCategory, action: string, data?: LogData): Promise<void> {
    ...
    try {
      ...
      await log(tabId, message)
    } catch (error) {
      console.error('[Logger] Failed to log directly:', error)
    }
  }
