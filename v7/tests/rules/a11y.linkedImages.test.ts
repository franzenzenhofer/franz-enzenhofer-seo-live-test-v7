...
const runAltRule = (html: string) =>
  linkedImagesAltRule.run({ html, url: 'https://example.com', doc: new DOMParser().parseFromString(html, 'text/html') } as any, { globals: {} })
...
describe('linked image rules', () => {
  ...
  it('passes when adequate alt', async () => {
    const result = await runAltRule('<a href="/"><img src="/x.png" alt="Logo"></a>')
    expect(result.type).toBe('ok')
  })

  it('passes when link text exists', async () => {
    const result = await runAltRule('<a href="/">Click<img src="/x.png"></a>')
    expect(result.type).toBe('ok')
  })
})
